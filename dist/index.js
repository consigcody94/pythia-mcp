 ...
 */
function validateCoupling(value, name) {
 ...
 */
function validateBranchingRatio(value, name) {
    if (value === undefined || value === null) {
        return NaN;
    }
 ...
 */
async function runLilith(args, input) {
 ...
 */
async function fetchHEPData(endpoint) {
 ...
 */
async function fetchCERNOpenData(endpoint) {
        ...
        switch (name) {
            ...
            case "list_experimental_data": {
                ...
                for (const line of lines) {
                    ...
                    if (experiment !== "all" && !trimmed.startsWith(experiment))
                        continue;
                    // Filter by run period
                    if (runPeriod !== "all" && !trimmed.includes(runPeriod))
                        continue;
                    datasets.push({
                        path: trimmed,
            ...
            case "scan_1d": {
                ...
                const scanResults = await parallelLimit(scanPoints, MAX_CONCURRENT_SCANS, async (point) => {
